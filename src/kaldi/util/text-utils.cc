...
bool StringsApproxEqualInternal(const char *a, const char *b,
                                int32_t decimal_places_tolerance,
                                int32_t places_into_number) {
start:
  char ca = *a, cb = *b;
  if (ca == cb) {
    if (ca == '\0') {
...
bool ConfigLine::GetValue(const std::string &key, int32_t *value) {
  KALDI_ASSERT(value != NULL);
  std::map<std::string, std::pair<std::string, bool> >::iterator it = data_.begin();
  for (; it != data_.end(); ++it) {
    if (it->first == key) {
...
bool ConfigLine::GetValue(const std::string &key, std::vector<int32_t> *value) {
  KALDI_ASSERT(value != NULL);
  value->clear();
  std::map<std::string, std::pair<std::string, bool> >::iterator it = data_.begin();
  for (; it != data_.end(); ++it) {
